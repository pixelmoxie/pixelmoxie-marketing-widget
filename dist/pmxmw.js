...
(function() {
  ...
  SelectorCache = function() {
    var clear, collection, getFresh, getFromCache;
    collection = {};
    getFromCache = function(selector) {
      if (collection[selector] == null) {
        collection[selector] = $(selector);
      }
      return collection[selector];
    };
    clear = function(selector) {
      collection[selector] = void 0;
    };
    getFresh = function(selector) {
      collection[selector] = void 0;
      return getFromCache(selector);
    };
    return {
      get: getFromCache,
      clear: clear,
...
(function() {
  ...
  PMXMW.WidgetView = (function(superClass) {
    ...
    WidgetView.prototype.initialize = function() {
      this.$window = selectors.get(window);
      this.$html = selectors.get('html');
      ...
      this.isLivePreview = window.location.href.indexOf('/customize/') >= 0 ? true : false;
      this.isFreeTheme = this.$body.hasClass('pixelmoxie-free-theme');
      this.showWidget = this.$body.hasClass('has-marketing-widget');
    };

    WidgetView.prototype.render = function() {
      var ajaxQuery, ajaxURL;
    ...
    WidgetView.prototype.loadCSS = function() {
    ...
    WidgetView.prototype.buildSettingsControls = function(controls) {
      ...
      pmxmwUnderscore.defer((function(_this) {
        return function() {
          (_this.$('.pmxWidgetModal-controls')).html(markup);
        };
      })(this));
    };
    ...
    WidgetView.prototype.layout = function() {
      ...
      this.$content.removeAttr('style').css({
        ...
        'top': 0,
        'left': 0,
        'width': this.viewportWidth * this.totalPanels,
        'height': this.viewportHeight
      });
      pmxmwUnderscore.each(this.$panels, (function(_this) {
        return function(elem, index) {
          ...
          $panel.css({
            'position': 'absolute',
            'top': 0,
            'left': _this.viewportWidth * index,
            'width': _this.viewportWidth,
            'height': _this.viewportHeight
          });
        };
      })(this));
      console.log("PMXMW: Viewport size is " + this.viewportWidth + "x" + this.viewportHeight);
    };

    WidgetView.prototype.showModal = function(event) {
      var $target, storageKey, triggeredIndex;
      ...
      if (triggeredIndex >= 0 && this.activeIndex !== triggeredIndex) {
        this.activeIndex = triggeredIndex;
        this.navigateTo(this.activeIndex, true);
      }
      this.$el.addClass('showModal');
      this.$body.addClass('show-marketing-modal');
    };

    WidgetView.prototype.hideModal = function() {
      this.$el.removeClass('showModal');
    ...
    WidgetView.prototype.navigateTo = function(index, snap) {
      var offset;
      if (snap == null) {
        snap = false;
      }
      this.$tabs.removeClass('isActive').eq(index).addClass('isActive');
      this.$panels.removeClass('isActive').eq(index).addClass('isActive');
      offset = this.viewportWidth * index * -1;
      if (snap) {
        this.$content.css({
          x: offset
        });
    ...
    WidgetView.prototype.demoSettingTriggered = function(event) {
      var eventHandle, eventValue;
      eventHandle = String(event.target.name || event.target.id).toLowerCase().replace(/ /g, '_');
      eventValue = 'checkbox' === event.target.type ? event.target.checked : String(event.target.id).toLowerCase();
      this.$el.trigger("pmxmw:" + eventHandle, [eventValue]);
    };

    WidgetView.prototype.setupEvents = function() {
      this.$window.on('resize orientationchange', pmxmwUnderscore.debounce(((function(_this) {
        return function() {
          _this.layout();
